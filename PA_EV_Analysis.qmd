...
PA_df %>%
  ...
  ggplot(aes(x = value, y = reorder(Metric, value), fill = Metric)) +
  ...
  geom_text(
    aes(label = if_else(
      value <= max(value) * .75, scales::comma(value, accuracy = 1), ""
    )),
    hjust = -.1,
  ...
  geom_text(
    aes(x = PA_df$`Rep Ballots Returned` + necessary_firewall, y = 0, label = "Firewall \n Threshold"),
    hjust = 1.1,
    vjust = 0,
    family = "Space Grotesk",
    size = 12 / .pt,
